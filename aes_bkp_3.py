...
def decrypt_file(key, in_filename, out_filename=None, chunksize=24*1024):
    ...
    """
    if not out_filename:
        out_filename = os.path.splitext(in_filename)[:-4]
...
if(choice == 'E' or choice == 'e'):
    ...
    pdw = hashlib.sha256(password.encode()).hexdigest()[:16]
    print("Password digest : ",pdw)
    aes = AES.new(pdw, AES.MODE_CBC, iv)
    ...
    for l in list_file:
    	...
    	decrypt_file(password, file_path, file_path[:-4])
    	#os.system('rm '+file_path)
